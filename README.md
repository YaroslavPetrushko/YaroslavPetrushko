# Hi, I'm Yaroslav Petrushko 👋

CS student from Ukraine · desktop apps, bots

---

## About me

- 🎓 Computer Science student
- 🔧 Backend developer — REST APIs, auth, data pipelines
- 🤝 Bootcamp 2026 contributor
- 📍 Ukraine
<!-- Add more... -->
---

## Tech stack

![C#](https://img.shields.io/badge/C%23-.NET%208-239120?style=flat&logo=csharp)
![Java](https://img.shields.io/badge/Java-JavaFX-b07219?style=flat&logo=openjdk)
![Node.js](https://img.shields.io/badge/Node.js-Express-339933?style=flat&logo=nodedotjs)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb)
![SQLite](https://img.shields.io/badge/SQLite-0F80CC?style=flat&logo=sqlite)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat&logo=postgresql)

---

## Projects

| Project | Description | Stack |
|---|---|---|
| [Fighting Games Tournament Tracker](https://github.com/YaroslavPetrushko/FightingTournament) | Desktop app for local fighting game events | C# .NET 8 · WPF · SQLite · MVVM |
| [CourseHub](https://github.com/YaroslavPetrushko/WEB_Server) | RESTful web server with blog & comments | Node.js · Express · MongoDB |
| [Java Book Manager](https://github.com/YaroslavPetrushko/OOP_Java) | JavaFX CRUD app with full test coverage | Java · JavaFX · PostgreSQL · JUnit 5 |

### 🚀 Bootcamp 2026 — [state-authorities-app](https://github.com/state-authorities-team/state-authorities-app/tree/main)

Contributed as a backend developer on a team project. Built a robust REST API and implemented secure authentication using Argon2id and JWT in httpOnly cookies. Co-engineered an autonomous, AI-driven news aggregation pipeline (Puppeteer / Cheerio).

`Node.js` `Express` `Prisma` `PostgreSQL` `Argon2id` `JWT` `Puppeteer` `Cheerio` `Google AI`

---

## How to reach me

- 📧 Email yaroslavpetrushko.off@gmail.com

[![Yaroslav's GitHub stats](https://github-readme-stats.vercel.app/api?username=yaroslavpetrushko&show_icons=true&theme=tokyonight&show=reviews,prs_merged,prs_merged_percentage)](https://github.com/anuraghazra/github-readme-stats)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=anuraghazra&layout=compact)

<!--
**YaroslavPetrushko/YaroslavPetrushko** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
